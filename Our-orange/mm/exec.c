...
PUBLIC int do_exec()
{
    ...
    if(STATIC_CHECK){
        // 检验校验和
        int checksum_save, checksum = 0;
        for (i = 0; i < NR_FILES; i++) // 读取预存的校验和
        {
            if (strcmp(pathname, check_table[i].filename) == 0)
                checksum_save = check_table[i].checksum;
        }
        for (i = 0; i < s.st_size; i++)
        {
            checksum ^= mmbuf[i] & 0xFF;
        }
        if (checksum == checksum_save)
        {
            printl("The file %s is safe.\n",pathname);
        }
        else
        {
            printl("The file %s is dangerous!\n",pathname);
            return -1;
        }
    }
